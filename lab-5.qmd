...
ggplot(births14_habitgiven, aes(x = habit, y = weight)) +
  geom_boxplot() +
  labs(
    title = "Birth Weight by Smoking Habit",
    x = "Mother's Smoking Habit",
    y = "Birth Weight (pounds)"
  )
```
Babies born to non-smoking mothers tend to have higher birth weights. The median birth weight for babies born to smoking mothers is shifted downwards suggesting that there may be a negative association between smoking and infant birth weight.
...
$$\hat{weight (lbs)} = 7.2699 - 0.5927*(\text{habitsmoker})$$
    ::: callout-tip
...
```{r}
model_age <- lm(weight ~ mage, data = births14_habitgiven)
summary(model_age)
```
...
Regression line is fit using the least squares method. Basically R is choosing the line that minimizes the sum of the squared differences between the actual and predicted birth weights.
...
The intercept is saying that the predicted birth weight is 6.7754 for a mother that is age 0. This is not meaningful because no mother is age 0.

d.  Interpret the slope in the context of the data and the research question.
For each additional year of the mother's age the baby's birth weight increases by roughly 0.01498, a slight positive association between maternal age and birth weight.
...
parsim because it measures the proportion of parasite shared across species.
...
Parasite similarity increases as divergence time decreases. So as species become separated for longer, they diverge more.
...
```{r}
model <- lm(parsim ~ divergence_time, data = parasites)
```
$$
\hat{parsim} = 0.379 -0.007* divergencetime
$$
b.  Interpret the slope and the intercept in the context of the data.
Intercept indicates that species that have not diverged have roughly a 0.379 proportion of parasites.
Slope indicates that for each additional million years since the species diverged, parasite similarity decreases by 0.007.
...
ggplot(parasites, aes(x = divergence_time, y = parsim)) +
  ...
  labs(
    x = "Divergence Time (mil years)",
    y = "Parasite Similarity",
    title = "Parasite Similarity vs Divergence Time (Regression)"
  )
```
...
As divergence time becomes greater, parasite similarity becomes negative. This is not possible biologically.
...
More distantly related species tend to share parasites. For each additional million years of divergence, the log odds of parasite similarity decreases by the slope.
...
coef(dt_model)["divergence_time"]
coef(dist_model)["distance"]
coef(BM_model)["BMdiff"]
coef(prec_model)["precdiff"]
```
...
It would not be directly useful since each slope depends on the unit/scale of its predictor variable. This means a difference in slope does not necessarily indicate a stronger or weaker effect.
...
r2_dt <- glance(dt_model)$r.squared
r2_dist <- glance(dist_model)$r.squared
r2_BM <- glance(BM_model)$r.squared
r2_prec <- glance(prec_model)$r.squared
...
Yes this would be useful since R^2 is unitless so you can compare across variables with different scales.
...
ggplot(gapminder_07, aes(x = gdpPercap, y = lifeExp)) +
  geom_point() +
  labs(
    x = "GDP per Capital (USD)",
    y = "Life Expectancy (years)",
    title = "Life Expectancy vs GDP per Capita (2007)"
  )

ggplot(gapminder_07, aes(x = gdpPercap_log, y = lifeExp))+ geom_point() +
  labs(
    x = "Log of GDP per Capita",
    y = "Life Expectancy (years)",
    title = "Life Expectancy vs Log of GDP per Capita (2007)"
  )
```
...
In the first scatter plot, the relationship is strongly non linear (steep rise then plateau). In the second scatter plot, the relationship is more gradual and closer to linear. The second relationship (log) would be better modeled using a linear model. This is because it reduces the curvature, making increases in life expectancy more constant for each proportional increase in GDP.
...
```{r}
gdp_model <- lm(gdpPercap_log ~ lifeExp, data = gapminder_07)
tidy(gdp_model)
```
...
```{r}
r2_glance <- glance(gdp_model)$r.squared
r2_glance
```
...
About 65.4% of the variation in log GDP per capita can be explained by differences in life expectancy.
...
color = continent)) + geom_point(size = 3, alpha = 0.7) +
  labs(
    x = "Log of GDP per Capita",
    y = "Life Expectancy (years)",
    title = "Life Expectancy vs Log of GDP per Capita by Continent (2007)"
  )
```
 Yes, there appears to be different trends that vary by continent. As per this scatter plot, Africa experiences lower life expectancy, Asia is more spread out, while other continents are clustered higher, experiencing longer life expectancy.
...
gdp_cont_model <- lm(gdpPercap_log ~ lifeExp + continent, data = gapminder_07)
tidy(gdp_cont_model)
```
...
e^3.2048 = 24.65
This model predicts that a GDP per capital of roughly $24.65 occurs if an African country has a life expectancy of 0 years.
...
(e^0.0781 - 1) * 100 = 8.12
For every added year of life expectancy, GDP per capita is expected to increase by roughly 8.12%
...
log_gdp_pred <- predict(gdp_cont_model, newdata = new_country)
log_gdp_pred

gdp_pred <- exp(log_gdp_pred)
gdp_pred
```
Asian country with a life expectancy of 70 years is predicted to have a GDP per capita of roughly $5906.74
...
I enjoyed this presentation because of its visual demonstration of worldwide data from over hundreds of years in such a short period and Rosling's enthusiasm also making it more of a narrative and engaging experience. For me, it was interesting to see how world wars or epidemics would cause a sudden break in the trend, showing many countries experiencing a decrease in life expectancy. At times the animation moves quickly and many bubbles overlap which can make it hard to follow a specific country. I think there's a few general take-aways from this presentation: all countries are progressing towards health and wealth but at different rates, countries in the same continent often cluster together, and the role time can play in a dynamic dataset like this one. My general take-away from this presentation is Rosling's ability to make a dataset engaging and easy to understand by general audiences because he is very enthusiast about the topic, adds an occasional joke, and paces the demonstration to keep viewers engaged.
