...
$$\hat{weight} = 7.2699 - 0.5927(\text{habitsmoker})$$
    ::: callout-tip
...
parsim because it measures the parasite similarity across species.
...
ggplot(parasites, aes(x = divergence_time, y = parsim)) +
  ...
  labs(
    ...
    title = "Parasite Similarty vs Divergence Time"
  )
```
...
```
$$
\hat{parsim} = 0.379 + -0.007\cdot divergencetime
$$
b.  Interpret the slope and the intercept in the context of the data.
Intercept indicates that species that have not diverged have roughly 0.379 proportion of parasites.
Slope indicates for each additional million years since the species diverged, parasite similarity decreases by 0.007.
...
More distantly related species are less likely to share parasites. For each additional million years of divergence, the log odds of parasite similarity decreases by the slope.
...
ggplot(gapminder_07, aes(x = gdpPercap, y = lifeExp)) +
  ...
  labs(
    ...
    title = "Life Expectancy vs GDP per Capital (2007)"
  )
...
In the first scatter plot, the relationship is mostly non linear (steep rise then plateau). In the second scatter plot, the relationship is more gradual and closer to linear. The second relationship (log) would be better modeled using a linear model. This is because it removes the curvature, making increases in life expectancy more constant for each proportional increase in GDP.
...
```{r}
ggplot(gapminder_07, aes(x = gdpPercap_log, y = lifeExp, color = continent)) + geom_point(size = 3, alpha = 0.7) +
  labs(
    x = "Log of GDP per Capita",
    y = "Life Expectancy (years)",
...
```
 Yes, there appears to be different trends that vary by continent. As per this scatter plot, Africa experiences lower life expectancy, while other continents are clustered higher, experiencing longer life expectancy.
...
(e^0.0781 - 1) * 100 = 8.12
For every added year of life expectancy, GDP per capital is expected to increasy by roughly 8.12%
